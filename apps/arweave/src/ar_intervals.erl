...
%% Only the intervals with the right bound smaller than UpperBound are considered for removal.
%% Return {Compacted, Intervals2} where Compacted is the list of freshly included intervals,
%% Intervals2 is the new set of Limit intervals.
%% If the set contains less intervals than Limit, return {[], Intervals}.
compact(Intervals, Limit, UpperBound) when Limit > 0 ->
	...
	case Excess =< 0 of
		...
		false ->
			{{infinity, _End}, I} =
				gb_sets:take_largest(inverse(cut(Intervals, UpperBound + 1))),
			I2 =
...
intervals_test() ->
	...
	{[], CI2_2} = compact(I2, 2, 10),
	compare(I2, CI2_2),
	{[], CI2_3} = compact(I2, 1, 20),
	compare(I2, CI2_3),
	...
	{[], CI3} = compact(I3, 2, 10),
	compare(I3, CI3),
	{[{3, 2}], CI3_2} = compact(I3, 1, 3),
	compare(add(new(), 6, 1), CI3_2),
	{[], CI3_3} = compact(I3, 1, 2),
	compare(I3, CI3_3),
	{[], CI3_4} = compact(I3, 1, 1),
	compare(I3, CI3_4),
	I3_2 = add(new(), 7, 5),
	...
	{[{9, 2}], C} = compact(add(add(new(), 2, 0), 10, 9), 1, 9),
	compare(add(new(), 10, 0), C),
	I6 = add(add(add(add(new(), 3, 1), 12, 8), 25, 22), 27, 26),
	{[], CI6} = compact(I6, 10, 27),
	compare(I6, CI6),
	{[], CI6_2} = compact(I6, 4, 27),
	compare(I6, CI6_2),
	{[{26, 25}], CI6_3} = compact(I6, 3, 27),
	compare(add(add(add(new(), 3, 1), 12, 8), 27, 22), CI6_3),
	{[{8, 3}, {26, 25}], CI6_4} = compact(I6, 2, 27),
	compare(add(add(new(), 12, 1), 27, 22), CI6_4),
	{[{22, 12}, {8, 3}, {26, 25}], CI6_5} = compact(I6, 1, 27),
